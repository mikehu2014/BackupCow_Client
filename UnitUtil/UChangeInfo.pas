...
{$Region ' 变化处理 ' }
  ...
  public
    constructor Create;
    procedure Sync( AThreadProc: TThreadProcedure );
  protected
...
{$Region ' Json 命令相关 ' }
  ...
  public
    ...
    class function getMsgStr( Msg : string ): string;
  public
...
procedure TMyFaceChange.Sync(AThreadProc: TThreadProcedure);
begin
  try
    if ChangeHandThreadList.Count > 0 then
      ChangeHandThreadList[0].Synchronize( AThreadProc );
  except
  end;
end;
...
class function MsgUtil.getMsgStr(Msg: string): string;
var
  MsgInfo : TMsgInfo;
begin
  MsgInfo := TMsgInfo.Create;
