...
type
  ...
  MyServerUtil = class
  public
    class function getShowName( CloudName, Ip : string ): string;
    class function getIp( ShowName : string ): string;
    class function getCloudName( ShowName : string ): string;
  end;
...
uses UMyUrl, URegisterInfoIO, ULanguageConst;
...
begin
  try  // 从系统 Api 中获取
    ...
    Result := FilePath;
    Result := Result + '\BackupCow_Cloud_Client';
...
begin
  ...
  if SplitList.Count >= 4 then
    Result := IntToStr( 17170 + StrToInt( SplitList[3] ) );
...
begin
  ...
  begin
    BaseInt := TimeTypeInt_Month;
    RightStr := LanguageInfo.frmMain_BackupShowTime_Months;
  end
  else
  if Mins >= TimeTypeInt_Week then
  begin
    BaseInt := TimeTypeInt_Week;
    RightStr := LanguageInfo.frmMain_BackupShowTime_Weeks;
  end
  else
  if Mins >= TimeTypeInt_Day then
  begin
    BaseInt := TimeTypeInt_Day;
    RightStr := LanguageInfo.frmMain_BackupShowTime_Days;
  end
  else
  if Mins >= TimeTypeInt_Hourse then
  begin
    BaseInt := TimeTypeInt_Hourse;
    RightStr := LanguageInfo.frmMain_BackupShowTime_Hours;
  end
  else
  begin
    BaseInt := TimeTypeInt_Minutes;
    RightStr := LanguageInfo.frmMain_BackupShowTime_Minutes;
  end;
...
begin
  ...
  begin
    ...
    RightStr := LanguageInfo.frmMain_BackupShowTime_Seconds;
  end;
...
begin
  if TimeType = TimeType_Minutes then
    Result := LanguageInfo.frmMain_BackupShowTime_Minutes
  else
  if TimeType = TimeType_Hourse then
    Result := LanguageInfo.frmMain_BackupShowTime_Hours
  else
  if TimeType = TimeType_Day then
    Result := LanguageInfo.frmMain_BackupShowTime_Days
  else
  if TimeType = TimeType_Week then
    Result := LanguageInfo.frmMain_BackupShowTime_Weeks
  else
  if TimeType = TimeType_Month then
    Result := LanguageInfo.frmMain_BackupShowTime_Months
  else
    Result := LanguageInfo.frmMain_BackupShowTime_Minutes;
end;
...
class function MyServerUtil.getCloudName(ShowName: string): string;
begin
  Result := Copy( ShowName, 1, Pos( ' (', ShowName ) - 1 );
  if Result = '' then
    Result := 'Cloud One';
end;

class function MyServerUtil.getIp(ShowName: string): string;
var
  l : Integer;
  s1 : string;
begin
  ...
  if l <= 2 then
    ...
    Result := Copy( ShowName, l, length( ShowName ) - l );
end;

class function MyServerUtil.getShowName(CloudName, Ip: string): string;
begin
  Result := CloudName + ' (' + Ip + ')';
end;
