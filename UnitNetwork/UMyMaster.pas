...
{$Region ' 信息 数据结构 ' }
  ...
  published
    ...
    property IsClient : Boolean Read iIsClient Write iIsClient;
  public
    ...
    procedure SetIsClient( _IsClient : Boolean );
  end;
  ...
  public
    ...
    procedure SetCloudName( _CloudName : string );
  published
    property Ip : string Read iIp Write iIp;
    property Port : string Read iPort Write iPort;
    property CloudName : string Read iCloudName Write iCloudName;
  end;


  TPingMsg = class( TMsgBase )
  private
    iPcID, iPcName, iCloudName : string;
    iIp, iPort : string;
    iIsLanConn : Boolean;
...
procedure TLanBroadcastMsg.SetIsClient(_IsClient: Boolean);
begin
  IsClient := _IsClient;
end;
...
begin
  ...
  if CloudIDNumberResult <> CloudIdNumberResult_OK then  // 子网不同
    Exit;

    // 是否已经连接 Server
  IsConnectServer := MyClient.IsConnServer;
  MySocketUtil.SendJsonStr( TcpSocket, JsonMsgType_IsConnectToServer_Confirm, IsConnectServer );
...
begin
  ...
  begin
    if IsLanConn then
      ...
      MyNetworkStatusApi.SetInternetSocketSuccess;
    NetworkServerIpApi.AddServer( Ip, CloudName );
  end;
end;
...
procedure TPingMsg.SetCloudName(_CloudName: string);
begin
...
procedure TMobileBroadcastReturnMsg.SetCloudName(_CloudName: string);
begin
  CloudName := _CloudName;
end;
