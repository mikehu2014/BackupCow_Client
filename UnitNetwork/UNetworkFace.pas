...
uses UChangeInfo, ComCtrls, ExtCtrls, ListActns, SysUtils, UMyUtil, Math, RzStatus, Classes,
     ...
     DateUtils, uDebug, Menus, StdCtrls, RzTabs, Controls, RzButton;
...
{$Region ' Server Login 界面 ' }

    // 父类
  TServerIpChangeFace = class( TFaceChangeInfo )
  public
    cbbLoginIp : TComboBox;
    tbtnSignupIp, tbtnLoginIp : TRzToolButton;
    pmSignup : TPopupMenu;
  protected
    procedure Update;override;
  private
    procedure AddItem( ShowStr : string );
  end;

    // 修改
  TServerIpWriteFace = class( TServerIpChangeFace )
  private
    Ip : string;
  protected
    CbbIndex : Integer;
  public
    constructor Create( _Ip : string );
  protected
    function FindCbbItem : Boolean;
  end;

    // 添加
  TServerIpAddFace = class( TServerIpWriteFace )
  protected
    procedure Update;override;
  end;

    // 移除
  TServerIpRemoveFace = class( TServerIpWriteFace )
  protected
    procedure Update;override;
  end;

    // 清空
  TServerIpClearFace = class( TServerIpChangeFace )
  protected
    procedure Update;override;
  end;

   // 设置帐号信息
  TAccountSetFace = class( TServerIpChangeFace )
  public
    Account, Password, ServerIp : string;
  public
    constructor Create( _Account, _Password, _ServerIp : string );
  protected
    procedure Update;override;
  end;

    // 设置 帐号名
  TAccountNameSetFace = class( TFaceChangeInfo )
  private
    Account : string;
  public
    constructor Create( _Account : string );
  protected
    procedure Update;override;
  end;

    // 重新登录
  TAccountLoginAgainFace = class( TFaceChangeInfo )
  protected
    procedure Update;override;
  end;

    // 读取 第一个 Ip
  TServerIpReadFirst = class( TServerIpChangeFace )
  public
    function get : string;
  end;
...
{$Region ' Server 无法连接 ' }
  ...
  TServerNotConnShowFace = class( TFaceChangeInfo )
  protected
    procedure Update;override;
  end;

  TServerNotConnHideFace = class( TFaceChangeInfo )
  protected
    procedure Update;override;
  end;
...
uses UMainForm, UFormUtil, UFormSetting,UNetworkControl, UFormLogin,
     UNetworkStatus,UFormEditionNotMatch, USettingInfo, ULanguageConst,
...
begin
  ...
  SbMyStatus.Caption := LanguageInfo.frmMain_SbMyStatus_NotConn;
...
begin
  ...
  ShowStr := LanguageInfo.frmMain_SbMyStatus_Conning;
  DotLen := t mod 4;
...
begin
  ...
  SbMyStatus.Caption := LanguageInfo.frmMain_SbMyStatus_Conn;
...
procedure TServerIpChangeFace.AddItem(ShowStr: string);
begin
//  frmLogin.AddIpToFace( ShowStr, True );
end;

procedure TServerIpChangeFace.Update;
begin
//  cbbSignupIp := frmLogin.cbbSignupServer;
//  cbbLoginIp := frmLogin.cbbLoginServer;
end;

{ TServerIpWriteFace }

constructor TServerIpWriteFace.Create(_Ip: string);
begin
  Ip := _Ip;
end;

function TServerIpWriteFace.FindCbbItem: Boolean;
var
  i: Integer;
begin
  Result := False;
//  for i := 0 to cbbSignupIp.Items.Count - 1 do
//    if cbbSignupIp.Items[i] = Ip then
//    begin
//      Result := True;
//      CbbIndex := i;
//      Break;
...
procedure TServerIpAddFace.Update;
begin
  inherited;
...
constructor TAccountSetFace.Create(_Account, _Password, _ServerIp: string);
begin
  Account := _Account;
  Password := _Password;
  ServerIp := _ServerIp;
...
procedure TAccountSetFace.Update;
begin
  inherited;
  ...
  frmMainForm.sbLogin.Caption := Account;
end;

{ TAccountNameSetFace }

constructor TAccountNameSetFace.Create(_Account: string);
begin
  Account := _Account;
end;

procedure TAccountNameSetFace.Update;
begin
  inherited;
  frmMainForm.sbLogin.Caption := Account;
end;

{ TAccountLoginAgainFace }

procedure TAccountLoginAgainFace.Update;
begin
  frmMainForm.HideMainForm;
...
procedure TServerIpClearFace.Update;
begin
  inherited;
//  cbbLoginIp.Clear;
//  cbbLoginIp.Items.Add( ItemCaption_Add );
//  cbbSignupIp.Clear;
//  cbbSignupIp.Items.Add( ItemCaption_Add );
end;

{ TServerIpRemoveFace }

procedure TServerIpRemoveFace.Update;
begin
  inherited;

  if not FindCbbItem then
    Exit;

  try
//    cbbSignupIp.Items.Delete( CbbIndex );
//    cbbLoginIp.Items.Delete( CbbIndex );
  except
  end;
end;
...
function TServerIpReadFirst.get: string;
begin
  inherited;

  Result := '';
//  if frmLogin.cbbLoginServer.Items.Count > 0 then
//    Result := frmLogin.cbbLoginServer.Items[0];
end;

{ TServerNotConnShowFace }

procedure TServerNotConnShowFace.Update;
begin
  inherited;
  frmMainForm.plNotConn.Visible := True;
end;

{ TServerNotConnHideFace }

procedure TServerNotConnHideFace.Update;
begin
  inherited;
  frmMainForm.plNotConn.Visible := False;
end;
