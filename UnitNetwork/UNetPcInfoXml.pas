...
{$Region ' 写 本机信息 Xml ' }
  ...
  protected
    ...
    // 设置帐号信息
  TMyPcInfoSetAccountXml = class( TMyPcInfoWriteXml )
  public
  ...
  protected
    ...
    // 设置帐号是否已登录
  TMyPcInfoSetIsLoginXml = class( TMyPcInfoWriteXml )
  public
    IsLogin : Boolean;
  public
...
{$Region ' 读取 帐号信息 ' }
  ...
  public
    ...
    ServerIp, AccountName : string;
  public
    constructor Create( _BackupList : IXMLNode );
    procedure SetAccountName( _ServerIp, _AccountName : string );
    procedure Update;
  end;
...
//  Xml_PcName = 'pn';
  ...
  Xml_InternetPort = 'ip';
  Xml_AccountName = 'ac';
  Xml_AccountPassword = 'ap';
  Xml_AccountServer = 'as';
  Xml_AccountServerSocket = 'ass';
  Xml_IsLogin = 'is';
...
begin
  ...
  Account := MyXmlUtil.GetChildValue( MyPcNode, Xml_AccountName );
  Password := MyXmlUtil.GetChildValue( MyPcNode, Xml_AccountPassword );
  ServerIp := MyXmlUtil.GetChildValue( MyPcNode, Xml_AccountServer );
  IsLogin := MyXmlUtil.GetChildBoolValue( MyPcNode, Xml_IsLogin );
  ServerSocket := MyXmlUtil.GetChildValue( MyPcNode, Xml_AccountServerSocket );
  if ServerSocket = '' then
    ServerSocket := ServerIp;
  ...
  begin
    ...
    MyPcInfoFirstSetHandle.SetIsLogin( False );
  ...
  begin
    ...
    MyPcInfoReadHandle.SetAccountInfo( Account, Password );
    MyPcInfoReadHandle.SetAccountServer( ServerIp, ServerSocket );
    MyPcInfoReadHandle.SetIsLogin( IsLogin );
    MyPcInfoReadHandle.Update;
    MyPcInfoReadHandle.Free;
  end;
end;

{ TMyPcInfoSetAccountXml }

constructor TMyPcInfoSetAccountXml.Create(_Account, _Password: string);
begin
  Account := _Account;
  Password := _Password;
end;

procedure TMyPcInfoSetAccountXml.SetServerInfo(_ServerIp,
...
procedure TMyPcInfoSetAccountXml.Update;
begin
  inherited;
  MyXmlUtil.AddChild( MyPcNode, Xml_AccountName, Account );
  MyXmlUtil.AddChild( MyPcNode, Xml_AccountPassword, Password );
  MyXmlUtil.AddChild( MyPcNode, Xml_AccountServer, ServerIp );
  MyXmlUtil.AddChild( MyPcNode, Xml_AccountServerSocket, ServerSocket );
end;
...
constructor TAccountWriteXml.Create( _ServerIp, _AccountName : string );
begin
  ServerIp := _ServerIp;
...
begin
  ...
  begin
    ...
    if ( MyXmlUtil.GetChildValue( SelectNode, Xml_AccountServer ) = ServerIp ) and
       ( MyXmlUtil.GetChildValue( SelectNode, Xml_AccountName ) = AccountName )
    then
    begin
      Result := True;
      AccountIndex := i;
...
begin
  ...
  MyXmlUtil.AddChild( AccountNode, Xml_AccountServer, ServerIp );
end;
...
procedure TAccountReadXml.ReadBackupList;
var
  BackupList : IXMLNode;
  i : Integer;
  BackupNode : IXMLNode;
...
begin
  ...
  begin
    ...
    BackupListReadXml.SetAccountName( ServerIp, AccountName );
...
begin
    // 提取信息
  ServerIp := MyXmlUtil.GetChildValue( AccountNode, Xml_AccountServer );
  AccountName := MyXmlUtil.GetChildValue( AccountNode, Xml_AccountName );

    // 处理读取
  AccountReadHandle := TAccountReadHandle.Create( ServerIp, AccountName );
...
function TBackupListWriteXml.FindBackupPath: Boolean;
var
  i : Integer;
  SelectNode : IXMLNode;
begin
...
constructor TBackupListReadXml.Create( _BackupList : IXMLNode );
begin
  BackupList := _BackupList;
end;

procedure TBackupListReadXml.SetAccountName(_ServerIp, _AccountName: string);
begin
  ServerIp := _ServerIp;
  AccountName := _AccountName;
end;
...
begin
  BackupPath := MyXmlUtil.GetChildValue( BackupList, Xml_BackupPath );

  BackupListReadHandle := TBackupListReadHandle.Create( ServerIp, AccountName );
...
{ TMyPcInfoSetIsLoginXml }

constructor TMyPcInfoSetIsLoginXml.Create(_IsLogin: Boolean);
begin
  IsLogin := _IsLogin;
end;

procedure TMyPcInfoSetIsLoginXml.Update;
begin
  inherited;
  MyXmlUtil.AddChild( MyPcNode, Xml_IsLogin, IsLogin );
end;

{ TMyAccountXmlRead }

procedure TMyAccountXmlRead.Update;
var
  MyAccountNode : IXMLNode;
  AccountNodeList : IXMLNode;
  i : Integer;
