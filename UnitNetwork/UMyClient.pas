...
uses Classes, Sockets, UChangeInfo, SyncObjs, UMyUtil, SysUtils,UMyNetPcInfo, DateUtils, UModelUtil,
     uDebug, UFileBaseInfo, uDebugLock, IdUDPClient;
...
{$Region ' Client 状态信息 ' }
  ...
  public
    ...
    // 其他服务器 Ip
  TPcNewServerIpMsg = class( TPcMsgBase )
  public
    iServerIp : string;
  published
    property ServerIp : string Read iServerIp Write iServerIp;
  public
    procedure SetServerIp( _ServerIp : string );
    procedure Update;override;
    function getMsgType : string;override;
  end;
...
{$Region ' Advance 网络 Pc 信息 ' }
  ...
  published
    ...
    property AccountName : string Read iAccountName Write iAccountName;
  public
    function getMsgType : string; override;
  end;
...
{$Region ' Account 同步信息 ' }
    ...
    // 父类
  TAccountChangeMsg = class( TMsgBase )
  public
    iAccount : string;
  published
    property Account : string Read iAccount Write iAccount;
  public
    ...
    // 设置 副本数
  TAccountSetCopyAccountMsg = class ( TAccountChangeMsg )
  public
    iCopyCOunt : Integer;
  published
  ...
  public
    function getMsgType : string;override;
      ...
      // 客户端帐号信息
  TAccountInfoMsg = class( TMsgBase )
  public
    iCopyCOunt : Integer;
  published
    property CopyCount : Integer Read iCopyCOunt Write iCopyCOunt;
  ...
  public
    ...
    // 客户端改密码信息
  TAccountChangePasswordMsg = class( TAccountChangeMsg )
  public
    iPassword : string;
  published
    property Password : string Read iPassword Write iPassword;
  ...
  public
    function getMsgType : string;override;
      ...
      // 信息工厂
  TAccountSyncMsgFactory = class( TMsgFactory )
  public
    constructor Create;
    function get: TMsgBase;override;
  end;
...
{$Region ' 断线广播 ' }
  ...
  public
    class procedure SendBroadcast;
    class procedure SendMsg( MsgStr : string );
    class procedure SendSetttingsMsg( MsgStr : string );
  public
    class procedure SendMsgIp( Ip, MsgStr : string );
    class procedure FindServerStr( MsgStr : string );
  end;
...
{$EndRegion}
  ...
  private        // 连接
    ...
    procedure WaitCloudPc; // 断开网络后等待Pc
  end;

var
  CloudPc_IsWaiting : Boolean = False;
...
const
  ...
  MsgType_PcStatus_AddServer = 'pst_as';
  ...
  MsgType_AccountSync = 'acs_';
  MsgType_AccountSync_SetCopyCount = 'acs_scc';
  MsgType_AccountSync_AccountInfo = 'acs_ai';
  MsgType_AccountSync_ChangePassword = 'acs_cp';
...
uses UMyServer,  UNetworkFace, UMyMaster, USearchServer, UMyTcp, UMyBackupApiInfo,
     ...
     UNetworkControl, UMyRestoreApiInfo, URestoreThread, UMainForm, UFormLogin,
...
begin
  ...
  MsgFactoryList.Add( MsgFactory );
    ...
    // 帐号同步
  MsgFactory := TAccountSyncMsgFactory.Create;
  MsgFactoryList.Add( MsgFactory );
end;
...
begin
  ...
  MySocketUtil.SendJsonStr( TcpSocket, JsonMsgType_Account_Enter, PcInfo.Account );
  ...
  PcOnlineMsg.SetAccount( PcInfo.Account );
  SendMsgToPc( ServerPcID, PcOnlineMsg );
end;
...
begin
  ...
  NetworkServerIpApi.RemoveServer( PcInfo.ServerIp );
...
procedure TMyClient.WaitCloudPc;
begin
...
begin
  ...
  NetworkPcResetHandle.Free;
    ...
    // 显示无法连接
  NetworkStatusApi.ShowServerNotConn;

    // 搜索网络 Pc
  CloudPc_IsWaiting := True;
  while IsRun and CloudPc_IsWaiting do
  begin
    SendBroadcastUtil.SendBroadcast;
    for i := 1 to 10 do
      if IsRun and CloudPc_IsWaiting then
        Sleep(500);
  end;

      // 隐藏无法连接
  NetworkStatusApi.HideServerNotConn;
end).Start;
end;
...
begin
  ...
  if MsgType = MsgType_PcStatus_AddServer then
    Result := TPcNewServerIpMsg.Create
  else
    Result := nil;
end;
...
begin
  ...
  if Account = PcInfo.Account then
...
procedure TServerInfoMsg.SetAccountName(_AccountName: string);
begin
...
function TPcNewServerIpMsg.getMsgType: string;
begin
  Result := MsgType_PcStatus_AddServer;
end;

procedure TPcNewServerIpMsg.SetServerIp(_ServerIp: string);
begin
  ServerIp := _ServerIp;
end;

procedure TPcNewServerIpMsg.Update;
begin
  inherited;
//  NetworkServerIpApi.AddServer( ServerIp );
end;


{ TAccountChangeMsg }

procedure TAccountChangeMsg.SetAccount(_Account: string);
begin
  Account := _Account;
end;

{ TAccountSetCopyAccountMsg }

function TAccountSetCopyAccountMsg.getMsgType: string;
begin
  Result := MsgType_AccountSync_SetCopyCount;
end;

procedure TAccountSetCopyAccountMsg.SetCopyCount(_CopyCount: Integer);
begin
  CopyCount := _CopyCount;
end;

procedure TAccountSetCopyAccountMsg.Update;
begin
  inherited;
end;

{ TAccountSyncMsgFactory }

constructor TAccountSyncMsgFactory.Create;
begin
  inherited Create( MsgType_AccountSync );
end;

function TAccountSyncMsgFactory.get: TMsgBase;
begin
  if MsgType = MsgType_AccountSync_SetCopyCount then
    Result := TAccountSetCopyAccountMsg.Create
  else
  if MsgType = MsgType_AccountSync_AccountInfo then
    Result := TAccountInfoMsg.Create
  else
    Result := nil;
end;
...
function TAccountInfoMsg.getMsgType: string;
begin
  Result := MsgType_AccountSync_AccountInfo;
end;

procedure TAccountInfoMsg.SetCopyCount(_CopyCount: Integer);
begin
  CopyCount := _CopyCount;
end;


procedure TAccountInfoMsg.Update;
begin
  inherited;
...
class procedure SendBroadcastUtil.FindServerStr(MsgStr: string);
var
  MobileBroadcastReturnMsg : TMobileBroadcastReturnMsg;
  ServerIp, ServerPort, CloudName : string;
begin
  if MsgStr = '' then
    Exit;
  ...
  try
    MobileBroadcastReturnMsg.SetMsgStr( MsgStr );
    ServerIp := MobileBroadcastReturnMsg.Ip;
    ServerPort := MobileBroadcastReturnMsg.Port;
    CloudName := MobileBroadcastReturnMsg.CloudName;
    if CloudName = ItemShow_CloudName then
    begin
      NetworkModeApi.EnterConnToPc( ServerIp, ServerPort );
      CloudPc_IsWaiting := False;
    end;
  except
  end;
...
class procedure SendBroadcastUtil.SendBroadcast;
var
  CloudIDNumMD5 : string;
  LanBroadcastMsg : TLanBroadcastMsg;
  MsgInfo : TMsgInfo;
  MsgType, MsgStr, Msg, InternetMsg : string;
...
begin
  ...
  LanBroadcastMsg.SetBroadcastType( BroadcastType_MobileSearch );
  LanBroadcastMsg.SetIsClient( True );
  MsgStr := LanBroadcastMsg.getMsgStr;
  LanBroadcastMsg.LanIp := PcInfo.InternetIp;
  LanBroadcastMsg.LanPort := PcInfo.InternetPort;
  InternetMsg := LanBroadcastMsg.getMsgStr;
  ...
  MsgType := IntToStr( ConnEdition_Now );
    ...
    // 包装 广播信息
  MsgInfo := TMsgInfo.Create;
  MsgInfo.SetMsgInfo( MsgType, InternetMsg );
  Msg := MsgInfo.getMsg;
  MsgInfo.Free;
  SendBroadcastUtil.SendSetttingsMsg( Msg );
end;
...
begin
  ...
  for i := 0 to IpList.Count - 1 do
    SendMsgIp( IpList[i], MsgStr );
  IpList.Free;
end;

class procedure SendBroadcastUtil.SendMsgIp(Ip, MsgStr: string);
var
  UdpClient : TIdUDPClient;
  RevStr : string;
begin
  ...
  try
    ...
    while True do
    begin
      RevStr := UdpClient.ReceiveString;
      if RevStr = '' then
        Break;
      FindServerStr( RevStr );
    end;
  except
  end;
  UdpClient.Free;
...
class procedure SendBroadcastUtil.SendSetttingsMsg(MsgStr: string);
var
  IpList : TStringList;
  i : Integer;
...
begin
    // 发送广播
  IpList := frmLogin.ReadIpList;
  for i := 0 to IpList.Count - 1 do
  begin
    IpStr := IpList[i];
    if not MyParseHost.IsIpStr( IpStr ) and
       not MyParseHost.HostToIP( IpStr, IpStr )
    then
      Continue;
    ...
    begin
      ...
      procedure
      begin
        frmLogin.RemoveIpFromFace( IpList[i] );  // 删除
        frmLogin.AddIpToFace( IpList[i], CloudName, False );
      end);
    end;
  end;
  IpList.Free;
...
{ TAccountChangePasswordMsg }

function TAccountChangePasswordMsg.getMsgType: string;
begin
  Result := MsgType_AccountSync_ChangePassword;
end;

procedure TAccountChangePasswordMsg.SetPassword(_Password: string);
begin
  Password := _Password;
end;

procedure TAccountChangePasswordMsg.Update;
begin
  inherited;
