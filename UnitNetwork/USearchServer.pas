...
{$Region ' 搜索网络 运行 ' }
  ...
  private
    ...
    function CheckLoginAccount : Boolean;
    function getIsConnectToCS : Boolean;
    procedure SetAccountBackupFace;
...
const
  ...
  AccountLoginResult_NotExist = 'AccountNotExist';
...
const
  ...
  ShowForm_AccountNameError : string = 'Account name "%s" does not exist.';
  ...
  UdpPort_Broadcast : Integer = 9542;
var
  MySearchMasterHandler : TMySearchMasterHandler;

implementation

uses UNetworkControl, UNetworkFace, UMyUtil, UMyMaster, UMyClient, UMyServer, UMyRegisterDataInfo,
     USettingInfo, uDebug, UNetPcInfoXml, UMyBackupDataInfo, UBackupThread, UCloudThread, URestoreThread,
     UChangeInfo, UMyTimerThread, UAppSplitEdition, UFormLogin, UMyBackupApiInfo, UFormNewSettings;
...
function TConnToPcSearchServer.CheckLoginAccount: Boolean;
var
  AccountLoginResult : string;
begin
    // 发送登录信息
  MySocketUtil.SendJsonStr( TcpSocket, JsonMsgType_AccountName, Account );
  MySocketUtil.SendJsonStr( TcpSocket, JsonMsgType_AccountPassword, Password );

    // 获取登录结果
  AccountLoginResult := MySocketUtil.RevJsonStr( TcpSocket );
  Result := AccountLoginResult = AccountLoginResult_Completed;
  if Result then  // 登录成功
  begin
    MyRegisterInfo.IsFreeLimit := MySocketUtil.RevJsonBool( TcpSocket );
    Exit;
  end;
...
begin
    // 帐号不存在
  if AccountLoginResult = AccountLoginResult_NotExist then
    MyMessageBox.ShowWarnning( Format( ShowForm_AccountNameError, [Account] ) )
  else   // 密码错误
  if AccountLoginResult = AccountLoginResult_PasswordError then
...
begin
  ...
  begin
      // 无法连接服务器
    MyMessageBox.ShowWarnning( 'Server cannot connect.' );
...
procedure TConnToPcSearchServer.SetAccountBackupFace;
begin
  DesItemAppApi.RefreshAccountFace;
end;

procedure TConnToPcSearchServer.SetAccountInfo(_Account, _Password: string);
begin
  Account := _Account;
  Password := _Password;
end;
...
begin
  ...
  begin
    ...
    // 设置已登录
  MyPcInfoApi.SetIsLogin( True );

    // 设置帐号备份界面
  SetAccountBackupFace;
...
begin
  ...
  begin
      // 10 秒钟 检测一次网络
    StartTime := Now;
    while not Terminated and ( SecondsBetween( Now, StartTime ) < 10 ) do
      Sleep(100);
    if Terminated then
      Break;
    SendBroadcast;
...
procedure TLanSearchPcThread.SendBroadcast;
begin
  SendBroadcastUtil.SendBroadcast;
end;
...
var
  ...
  i: Integer;
begin
  Port := StrToIntDef( PcInfo.LanPort, 9393 );
  for i := 0 to 10000 do
  begin
      // 端口可用则结束
    if MyTcpUtil.getPortAvaialble( Port ) then
...
begin
  ...
  MsgStr := MsgUtil.getMsgStr( MsgStr );
  ...
  try
    ...
    NetworkServerIpApi.AddServer( ServerIp, CloudName );
  except
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      procedure
      begin
        frmLogin.RemoveInputIpFromFace( IpList[i] );  // 删除
