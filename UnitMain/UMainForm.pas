...
uses
  ...
  RzTabs, Grids, ValEdit, ActnCtrls, UFileBaseInfo, ULanguageUtil,
  VCLTee.Series, VCLTee.TeEngine, VCLTee.TeeProcs, UMyUtil, VCLTee.Chart, htmlhint,
  uDebug,Generics.Collections;

const
  hfck = wm_user + $1000;
  AppName_FileCloud = 'BackupCow_Cloud_Client';
...
type
  ...
  TfrmMainForm = class(TForm)
    ...
    Account1: TMenuItem;
    sbLogin: TRzGlyphStatus;
    tmrExit: TTimer;
    sbLogout: TRzToolButton;
    tbtnCopyCount: TToolButton;
    sbCopyCount: TRzGlyphStatus;
    ...
    Label16: TLabel;
    Label17: TLabel;
    btnSaveLanguage: TButton;
    btnLoadEnglish: TButton;
    btnLoadChinese: TButton;
    ...
    procedure tbtnCopyCountClick(Sender: TObject);
    procedure btnSaveLanguageClick(Sender: TObject);
    procedure btnLoadEnglishClick(Sender: TObject);
    procedure btnLoadChineseClick(Sender: TObject);
  public
    procedure LoadLangauge( Language : string );
  public
...
const
  ...
  VstBackup_BackupEncrypted = 1;
  VstBackup_FileCount = 2;
  VstBackup_FileSize = 3;
  VstBackup_NextBackup = 4;
  VstBackup_Percentage = 5;
  VstBackup_Status = 6;

  VstRestore_RestoreName = 0;
  VstRestore_RestorreEnctypted = 1;
  VstRestore_RestoreOwner = 2;
  VstRestore_FileCount = 3;
  VstRestore_FileSize = 4;
  VstRestore_LastBackupTime = 5;
...
var  // 显示的 Cloud Name
  ItemShow_CloudName : string = 'Cloud One';
...
uses
  ...
  UNetworkFace, UNetworkControl, ULanguageConst,
  UMyNetPcInfo, UFormSetting, UMyUrl, UFormNewSettings,
  UFormAbout, UFormLogin, UFormLogout,
...
procedure TfrmMainForm.Account1Click(Sender: TObject);
begin
  if PcInfo.IsLogin then
  begin
    frmLogout.SetAccountInfo( PcInfo.ServerIp, PcInfo.Account );
    frmLogout.Show;
  end
  else
    frmLogin.Show;
...
begin
  try
    IsHide := btnHide.Tag = 10;
    slRestoreDown.Visible := not IsHide;
    if IsHide then
    begin
      btnHide.Caption := LanguageInfo.frmMain_BtnRestore_Show;
    ...
    begin
      btnHide.Caption := LanguageInfo.frmMain_BtnRestore_Hide;
...
procedure TfrmMainForm.btnLoadChineseClick(Sender: TObject);
begin
  LoadLangauge( Language_Chinese );
end;

procedure TfrmMainForm.btnLoadEnglishClick(Sender: TObject);
begin
  LoadLangauge( Language_English );
end;
...
procedure TfrmMainForm.btnSaveLanguageClick(Sender: TObject);
var
  SavePath : string;
begin
  SavePath := 'D:\work\client\English.ini';
  FormLanguageUtil.Save( Self, SavePath );
  FormLanguageUtil.Save( frmLogin, SavePath );
  FormLanguageUtil.Save( frmLogout, SavePath );
  FormLanguageUtil.Save( frmNewSettings, SavePath );
  FormLanguageUtil.Save( frmAbout, SavePath );
  FormLanguageUtil.Save( frmSelectBackupItem, SavePath );
  FormLanguageUtil.Save( frmRestoreExplorer, SavePath );
  FormLanguageUtil.Save( frmSelectRestore, SavePath );
  FormLanguageUtil.Save( frmDecrypt, SavePath );
  TrayIconLanguageUtil.Save( tiApp, SavePath );
  LanguageInfo.Save( SavePath );
end;
...
begin
  if frmNewSettings.LastLanguageIndex = LanguageIndex_Chinese then
    btnLoadChinese.Click;
  if LanguageEdition_IsDebug then
    plBackupTitle.Visible := True;
...
procedure TfrmMainForm.LoadLangauge(Language: string);
var
  LoadPath : string;
begin
  if Language = Language_English then
  begin
    if LanguageEdition_IsDebug then
      LoadPath := 'D:\work\client\English.ini'
    else
      LoadPath := ExtractFilePath( Application.ExeName ) + 'Language\English.ini'
  end
  else
  if Language = Language_Chinese then
  begin
    if LanguageEdition_IsDebug then
      LoadPath := 'D:\work\client\Chinese.ini'
    else
      LoadPath := ExtractFilePath( Application.ExeName ) + 'Language\Chinese.ini';
  end;
  FormLanguageUtil.Load( Self, LoadPath );
  FormLanguageUtil.Load( frmLogin, LoadPath );
  FormLanguageUtil.Load( frmLogout, LoadPath );
  FormLanguageUtil.Load( frmNewSettings, LoadPath );
  FormLanguageUtil.Load( frmAbout, LoadPath );
  FormLanguageUtil.Load( frmSelectBackupItem, LoadPath );
  FormLanguageUtil.Load( frmRestoreExplorer, LoadPath );
  FormLanguageUtil.Load( frmSelectRestore, LoadPath );
  FormLanguageUtil.Load( frmDecrypt, LoadPath );
  TrayIconLanguageUtil.Load( tiApp, LoadPath );
  LanguageInfo.Load( LoadPath );
end;
...
begin
  try
    ...
    RestorePcFilter := iniFile.ReadInteger(frmMainForm.Name, pmRestorePcFilter.Name, 2 );
    iniFile.Free;
...
procedure TfrmMainForm.sbLogoutClick(Sender: TObject);
begin
    // 弹出logout窗口
  frmLogout.SetAccountInfo( ItemShow_CloudName, PcInfo.Account );
  frmLogout.Show;
end;
...
procedure TfrmMainForm.tbtnCopyCountClick(Sender: TObject);
var
  InputStr : string;
  CopyCount : Integer;
begin
  InputStr := sbCopyCount.Caption;
  if not InputQuery( 'Input', 'Copy Quantity', InputStr ) then
    Exit;
  CopyCount := StrToIntDef( InputStr, 2 );
  NetworkAccountApi.SetRemoteCopyCount( CopyCount );
...
begin
  PcMain.ActivePageIndex := (Sender as TRzToolButton).Tag mod 10;
end;
...
begin
    // 正在登录
  if frmLogin.Showing then
  begin
    ShowWindow(frmLogin.Handle, SW_RESTORE);
...
procedure TfrmMainForm.tmrExitTimer(Sender: TObject);
begin
...
begin
  ...
  SourceItemList := TStringList.Create;
    ...
    // 寻找选中的源
  SelectNode := VstBackup.GetFirstVisible;
  if Assigned( SelectNode ) then
  begin
    NodeData := VstBackup.GetNodeData( SelectNode );
    DesItemList.Add( NodeData.ItemID );
...
begin
  ...
  begin
    ...
    begin
      ...
      NetworkAccountApi.RemoveAccountPath( PcInfo.ServerIp, PcInfo.Account, NodeData.ItemID );
    end;
    SelectNode := VstBackup.GetNextSelected( SelectNode );
  end;
end;
...
begin
  ...
  frmNewSettings.ResetLimit( IsLimit, LimitValue, LimitType );
  frmNewSettings.SetAcccount( PcInfo.Account );
  frmNewSettings.ShowNormalSetting;
end;
...
begin
  ...
  tbtnBackupRemove.Enabled := IsChild;
end;
...
begin
  ...
  begin
    ...
    if ( Column = VstBackup_BackupEncrypted ) and NodeData.IsEncrypted then
        ImageIndex := MyShellTransActionIconUtil.getEncrypted
    else
    if Column = VstBackup_Status then
    begin
...
begin
  ...
  begin
    if Column = VstBackup_BackupName then
    begin
      CellText := 'Backup to ' + ItemShow_CloudName;
    end
    else
  ...
  try   // 鼠标定位
    ...
    begin
      ...
      if VstBackupUtil.getIsBackupNode( NodeData.NodeType ) then
        ...
        HintStr := VstBackupUtil.getDesHintStr( SelectNode )
    end
    else
      HintStr := LanguageInfo.frmMain_DropFile_Hint;
  except
    HintStr := '';
  end;
...
begin
  ...
  begin
    ...
    if ( Column = VstRestore_RestorreEnctypted ) and NodeData.IsEncrypted then
      ImageIndex := MyShellTransActionIconUtil.getEncrypted;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      CellText := NodeData.ShowName;
      if Node.Parent = Sender.RootNode then
        CellText := 'Backups stored in ' + ItemShow_CloudName;
    end
    else
    if Node.Parent = Sender.RootNode then
    begin
...
begin
  ...
  for i := 0 to NetworkDesList.Count - 1 do
    ...
    begin
      ...
      NetworkAccountApi.AddAccountPath( PcInfo.ServerIp, PcInfo.Account, BackupPathList[j] );
