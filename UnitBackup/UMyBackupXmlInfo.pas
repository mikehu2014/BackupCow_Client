...
uses UChangeInfo, xmldom, XMLIntf, msxmldom, XMLDoc, UXmlUtil, UFileBaseInfo, UMyUtil, SysUtils,
     uDebug, classes;
...
{$Region ' 数据修改 ' }
  ...
  protected
    ...
    // 设置
  TBackupItemFileExtSetXml = class( TBackupItemWriteXml )
  private
    FileExtList : TStringList;
  public
    procedure SetNewFileExtList( NewFileExtList : TStringList );
...
{$Region ' 数据读取 ' }
  ...
  public
    ...
    // 读取 备份文件后缀列表
  TBackupItemFileExtReadXml = class
  public
    DesItemID : string;
    BackupPath : string;
    FileExtListNode : IXMLNode;
  public
    constructor Create( _FileExtListNode : IXMLNode );
    procedure SetDesItemID( _DesItemID : string );
    procedure SetBackupPath( _BackupPath : string );
    procedure Update;
  end;
...
procedure TBackupItemReadXml.ReadFileExtList;
var
  FileExtListNode : IXMLNode;
  BackupItemFileExtReadXml : TBackupItemFileExtReadXml;
begin
  FileExtListNode := MyXmlUtil.AddChild( BackupItemNode, Xml_FileExtList );
  BackupItemFileExtReadXml := TBackupItemFileExtReadXml.Create( FileExtListNode );
  BackupItemFileExtReadXml.SetDesItemID( DesItemID );
  BackupItemFileExtReadXml.SetBackupPath( BackupPath );
  BackupItemFileExtReadXml.Update;
  BackupItemFileExtReadXml.Free;
end;
...
{ TBackupItemFileExtSetXml }

destructor TBackupItemFileExtSetXml.Destroy;
begin
  FileExtList.Free;
  inherited;
end;

procedure TBackupItemFileExtSetXml.SetNewFileExtList(
  NewFileExtList: TStringList);
var
  i: Integer;
begin
  FileExtList := TStringList.Create;
  for i := 0 to NewFileExtList.Count - 1 do
    FileExtList.Add( NewFileExtList[i] );
end;

procedure TBackupItemFileExtSetXml.Update;
var
  FileExtListNode, FileExtNode : IXMLNode;
  i: Integer;
begin
  inherited;
  ...
  if not FindBackupItemNode then
    Exit;
  FileExtListNode := MyXmlUtil.AddChild( BackupItemNode, Xml_FileExtList );
  FileExtListNode.ChildNodes.Clear;
  for i := 0 to FileExtList.Count - 1 do
  begin
    FileExtNode := MyXmlUtil.AddListChild( FileExtListNode );
    MyXmlUtil.AddChild( FileExtNode, Xml_FileExt, FileExtList[i] );
  end;
end;

{ TBackupItemFileExtReadXml }

constructor TBackupItemFileExtReadXml.Create(_FileExtListNode: IXMLNode);
begin
  FileExtListNode := _FileExtListNode;
end;

procedure TBackupItemFileExtReadXml.SetBackupPath(_BackupPath: string);
begin
  BackupPath := _BackupPath;
end;

procedure TBackupItemFileExtReadXml.SetDesItemID(_DesItemID: string);
begin
  DesItemID := _DesItemID;
end;

procedure TBackupItemFileExtReadXml.Update;
var
  FileExtList : TStringList;
  i: Integer;
  FileExtNode : IXMLNode;
  FileExt : string;
  BackupItemFileExtReadHandle : TBackupItemFileExtReadHandle;
begin
    // 读取信息
  FileExtList := TStringList.Create;
  for i := 0 to FileExtListNode.ChildNodes.Count - 1 do
  begin
    FileExtNode := FileExtListNode.ChildNodes[i];
    FileExt := MyXmlUtil.GetChildValue( FileExtNode, Xml_FileExt );
    FileExtList.Add( FileExt );
  end;

    // 处理信息
  BackupItemFileExtReadHandle := TBackupItemFileExtReadHandle.Create( DesItemID );
  BackupItemFileExtReadHandle.SetBackupPath( BackupPath );
  BackupItemFileExtReadHandle.SetFileExtList( FileExtList );
  BackupItemFileExtReadHandle.Update;
  BackupItemFileExtReadHandle.Free;
  FileExtList.Free;
end;
