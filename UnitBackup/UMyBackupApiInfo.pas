...
{$Region ' 源路径 过滤信息 ' }
  ...
  private
    ...
    // 读取 后缀过滤
  TBackupItemFileExtReadHandle = class( TBackupItemWriteHandle )
  private
    FileExtList : TStringList;
  public
    procedure SetFileExtList( _FileExtList : TStringList );
    procedure Update;virtual;
  private
    procedure SetToInfo;
  end;
    ...
    // 设置 后缀过滤
  TBackupItemFileExtSetHandle = class( TBackupItemFileExtReadHandle )
  public
    procedure Update;override;
  private
    procedure SetToXml;
...
{$EndRegion}
  ...
  public
    class procedure RefreshAccountFace;
    class procedure HideBackupBtn;
  end;
  ...
  public              // 修改 过滤器
    ...
    class procedure SetFileExtList( DesItemID, BackupPath : string; FilterList : TStringList );
  public              // 备份操作
...
uses UMyBackupDataInfo, UMyBackupFaceInfo, UMyBackupXmlInfo, UMyNetPcInfo, UMyUtil, UBackupThread,
     ...
     UFormBackupLog, UXmlUtil, XMLIntf, UChangeInfo, UMainFormFace, UFrmSelectBackupItem, ULanguageConst;
...
class procedure BackupItemUserApi.SetFileExtList(DesItemID,
  BackupPath: string; FilterList: TStringList);
var
  BackupItemFileExtSetHandle : TBackupItemFileExtSetHandle;
begin
  BackupItemFileExtSetHandle := TBackupItemFileExtSetHandle.Create( DesItemID );
  BackupItemFileExtSetHandle.SetBackupPath( BackupPath );
  BackupItemFileExtSetHandle.SetFileExtList( FilterList );
  BackupItemFileExtSetHandle.Update;
  BackupItemFileExtSetHandle.Free;
end;
...
begin
  ...
  SetBackupItemStatus( DesItemID, BackupPath, LanguageInfo.frmMain_BackupNodeStatus_Backuping );
end;
...
class procedure BackupItemAppApi.SetWaitingBackup(DesItemID,
  BackupPath: string);
begin
  SetBackupItemStatus( DesItemID, BackupPath, LanguageInfo.frmMain_BackupNodeStatus_WaitingBackup );
end;
...
begin
    // 帐号不存在路径
  if not AccountBackupReadUtil.ReadBackupPathIsExist( PcInfo.Account, BackupPath ) then
    Exit;
...
class procedure DesItemAppApi.HideBackupBtn;
var
  AccountHideBackupBtnFace : TAccountHideBackupBtnFace;
begin
  AccountHideBackupBtnFace := TAccountHideBackupBtnFace.Create;
  AccountHideBackupBtnFace.AddChange;
end;

class procedure DesItemAppApi.RefreshAccountFace;
var
  NetworkPathList, AccountPathList : TStringList;
  DesItemSetAccountFace : TDesItemSetAccountFace;
  i: Integer;
  j: Integer;
  BackupConfigInfo : TBackupConfigInfo;
begin
  AccountPathList := AccountBackupReadUtil.ReadAccountPathList( PcInfo.Account );

  BackupConfigInfo := TBackupConfigInfo.Create;
  BackupConfigInfo.IsBackupupNow := True;
  BackupConfigInfo.IsAuctoSync := True;
  BackupConfigInfo.SyncTimeType := TimeType_Hourse;
  BackupConfigInfo.SyncTimeValue := 1;
  BackupConfigInfo.IsEncrypt := False;
  BackupConfigInfo.IsKeepDeleted := True;
  BackupConfigInfo.KeepEditionCount := 3;
  BackupConfigInfo.IncludeFilterList := TFileFilterList.Create;
  BackupConfigInfo.ExcludeFilterList := TFileFilterList.Create;
  NetworkPathList := DesItemInfoReadUtil.ReadNetworkDesList;
  for i := 0 to NetworkPathList.Count - 1 do
    for j := 0 to AccountPathList.Count - 1 do;
//      BackupItemUserApi.AddItem( NetworkPathList[i], AccountPathList[j], BackupConfigInfo );
  NetworkPathList.Free;
  BackupConfigInfo.Free;

  DesItemSetAccountFace := TDesItemSetAccountFace.Create;
  for i := 0 to AccountPathList.Count - 1 do
    DesItemSetAccountFace.AddPath( AccountPathList[i] );
  DesItemSetAccountFace.AddChange;
...
var
  ...
  PcID, PcName, Directory, IpStr, DesName : string;
begin
  ...
  IpStr := MyNetPcInfoReadUtil.ReadIp( PcID );
  DesName := MyNetPcInfoReadUtil.ReadName( PcID );
  ...
  DesItemAddNetworkFace.SetIp( IpStr, DesName );
...
begin
  ...
  BackupItemAppApi.SetIsBackuping( DesItemID, BackupPath, False );
    ...
    // 设置 状态为空
  BackupItemAppApi.SetBackupItemStatus( DesItemID, BackupPath, LanguageInfo.frmMain_BackupNodeStatus_Empty );
end;
...
begin
  Params.ErrorStatus := LanguageInfo.frmMain_BackupNodeStatus_LostConnectError;
  AddItem( Params );
end;

class procedure BackupErrorAppApi.ReadFileError(Params: TBackupErrorAddParams);
begin
  Params.ErrorStatus := LanguageInfo.frmMain_BackupNodeStatus_ReadFileError;
  AddItem( Params );
end;

class procedure BackupErrorAppApi.SendFileError(Params: TBackupErrorAddParams);
begin
  Params.ErrorStatus := LanguageInfo.frmMain_BackupNodeStatus_SendFileError;
  AddItem( Params );
end;

class procedure BackupErrorAppApi.WriteFileError(Params: TBackupErrorAddParams);
begin
  Params.ErrorStatus := LanguageInfo.frmMain_BackupNodeStatus_WriteFileError;
  AddItem( Params );
end;
...
begin
  ...
  BackupItemAppApi.SetScaningCount( DesItemID, BackupPath, 0 );
    ...
    // 设置状态 正在分析
  BackupItemAppApi.SetBackupItemStatus( DesItemID, BackupPath, LanguageInfo.frmMain_BackupNodeStatus_Analyizing );
end;
...
{ TBackupItemExtFilterSetHandle }

procedure TBackupItemFileExtSetHandle.SetToXml;
var
  BackupItemFileExtSetXml : TBackupItemFileExtSetXml;
begin
  BackupItemFileExtSetXml := TBackupItemFileExtSetXml.Create( DesItemID );
  BackupItemFileExtSetXml.SetBackupPath( BackupPath );
  BackupItemFileExtSetXml.SetNewFileExtList( FileExtList );
  BackupItemFileExtSetXml.AddChange;
end;

procedure TBackupItemFileExtSetHandle.Update;
begin
  inherited;
  SetToXml;
end;

{ TBackupItemExtFilterReadHandle }

procedure TBackupItemFileExtReadHandle.SetFileExtList(
  _FileExtList: TStringList);
begin
  FileExtList := _FileExtList;
end;

procedure TBackupItemFileExtReadHandle.SetToInfo;
var
  BackupItemFileExtSetInfo : TBackupItemFileExtSetInfo;
begin
  BackupItemFileExtSetInfo := TBackupItemFileExtSetInfo.Create( DesItemID );
  BackupItemFileExtSetInfo.SetBackupPath( BackupPath );
  BackupItemFileExtSetInfo.SetNewFileExtList( FileExtList );
  BackupItemFileExtSetInfo.Update;
  BackupItemFileExtSetInfo.Free;
end;

procedure TBackupItemFileExtReadHandle.Update;
begin
  SetToInfo;
end;
