...
type
  ...
  private       // 读取
    ...
    procedure SetDefaultDestination; // 设置默认目标
  public        // 添加/属性
...
uses
  ...
  UMyBackupApiInfo, UMyBackupFaceInfo, UMyNetPcInfo, ULanguageConst;
...
begin
  cbbSyncTime.Clear;
  cbbSyncTime.Items.Add( LanguageInfo.frmSelectBackup_AutoBackup_Min );
  cbbSyncTime.Items.Add( LanguageInfo.frmSelectBackup_AutoBackup_Hour );
  cbbSyncTime.Items.Add( LanguageInfo.frmSelectBackup_AutoBackup_Day );
  cbbSyncTime.Items.Add( LanguageInfo.frmSelectBackup_AutoBackup_Week );
  cbbSyncTime.Items.Add( LanguageInfo.frmSelectBackup_AutoBackup_Month );
  cbbSyncTime.ItemIndex := 1;
...
procedure TfrmSelectBackupItem.btnNewCancelClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmSelectBackupItem.btnNewOKClick(Sender: TObject);
begin
    // 设置默认目标
...
var
  IsEnable : Boolean;
begin
  IsEnable := vstSelectPath.CheckedCount > 0;
...
procedure TfrmSelectBackupItem.SetDefaultDestination;
var
  SelectNode : PVirtualNode;
  NodeData : PShellObjectData;
begin
  if vstNetworkDes.CheckedCount > 0 then
    Exit;
  if vstNetworkDes.RootNodeCount <= 0 then
    Exit;

  SelectNode := vstNetworkDes.GetFirstVisible;
...
begin
  ...
  IsAddItem := True;
  Self.Caption := LanguageInfo.frmSelectBackup_Title_AddItem;
  ...
  CheckBtnOkEnable;
    ...
    // 是否已选中
  btnNewOK.Enabled := SourceItemList.Count > 0;
...
begin
  ...
  IsAddItem := False;
  Self.Caption := Format( LanguageInfo.frmSelectBackup_Title_ItemProperties, [ MyFileInfo.getFileName( SourcePath ) ] );
...
begin
  ...
  btnNewOK.Enabled := Sender.CheckedCount > 0;
  CheckBtnOkEnable;
end;
