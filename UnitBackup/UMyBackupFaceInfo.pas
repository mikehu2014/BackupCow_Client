...
{$Region ' 数据结构 ' }
  ...
  public
    IpStr, DesPcName : WideString; // 云节点信息
  public
...
{$Region ' 备份信息 数据修改 ' }
  ...
  public
    ...
    procedure SetIp( _Ip, _DesPcName : string );
  protected
    procedure CreateDesItem;override;
    procedure SetDesItemInfo;override;
    procedure ResetItemInfo;override;
  ...
  TDesItemSetAccountFace = class( TDesItemChangeFace )
  public
    AccountPathList : TStringList;
  public
    constructor Create;
  ...
  protected
    ...
    // 关闭备份按钮
  TAccountHideBackupBtnFace = class( TFaceChangeInfo )
  protected
    procedure Update;override;
  end;
...
{$Region ' 数据读取 ' }
  ...
  public             // 获取 Hint 信息
    ...
    class function getDesHintStr( Node : PVirtualNode ): string;
  public             // 节点类型
...
uses UMainForm, UIconUtil, UFrmSelectBackupItem, UFormBackupLog, UMyBackupApiInfo, UFormBackupPcFilter,
     UFormBackupHint, ULanguageConst;
...
begin
  ...
  begin
    ...
    begin
      if VstBackup.IsVisible[ ChildNode ] then
        Exit;
...
begin
  ...
  if NodeData.IsCompleted then
    Result := LanguageInfo.frmMain_BackupStatusShow_Completed
  else
  if NodeData.IsDesBusy then
    Result := LanguageInfo.frmMain_BackupStatusShow_Busy
  else
  if not NodeData.IsExist then
    Result := LanguageInfo.frmMain_BackupStatusShow_NotExist
  else
  if NodeData.NodeStatus = LanguageInfo.frmMain_BackupNodeStatus_Analyizing then
  begin
    if NodeData.AnalyizeCount <= 0 then
      Result := LanguageInfo.frmMain_BackupNodeStatus_Analyizing
    else
      Result := Format( LanguageInfo.frmMain_BackupStatusShow_Analyizing, [  MyCount.getCountStr( NodeData.AnalyizeCount ) ] );
  end
  else
  if NodeData.NodeStatus = LanguageInfo.frmMain_BackupNodeStatus_Backuping then
  begin
    Result := MyPercentage.getPercentageStr( NodeData.Percentage ) + '   ';
    if NodeData.Speed <= 0 then
      Result := Result + LanguageInfo.frmMain_BackupNodeStatus_Backuping
    else
      Result := Result + MySpeed.getSpeedStr( NodeData.Speed )
  end
  else
  if NodeData.NodeStatus <> '' then
    Result := NodeData.NodeStatus
  else
    Result := LanguageInfo.frmMain_BackupStatusShow_InCompleted;
end;
...
begin
  ...
  if NodeData.NodeStatus = LanguageInfo.frmMain_BackupNodeStatus_WaitingBackup then
    Result := MyShellTransActionIconUtil.getWaiting
  else
  if NodeData.NodeStatus = LanguageInfo.frmMain_BackupNodeStatus_Analyizing then
...
class function VstBackupUtil.getDesHintStr(Node: PVirtualNode): string;
var
  NodeData : PVstBackupData;
begin
  NodeData := frmMainForm.VstBackup.GetNodeData( Node );
  Result := MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_ComputerName, NodeData.DesPcName );
  Result := Result + MyHtmlHintShowStr.getHintRow( LanguageInfo.frmMain_BackupHint_ComputerIP, NodeData.IpStr );
end;
...
begin
  ...
  if not NodeData.IsWrite then
    Result := LanguageInfo.frmMain_BackupStatusShow_NotWrite
  else
  if NodeData.IsLackSpace then
    Result := LanguageInfo.frmMain_BackupStatusShow_NotSpace
  else
  if not NodeData.IsOnline then
    Result := LanguageInfo.frmMain_BackupStatusShow_PcOffline
  else
  if not NodeData.IsConnected then
    Result := LanguageInfo.frmMain_BackupStatusShow_NotConnect
  else
...
begin
  ...
  NodeData.NextSyncTimeShow := LanguageInfo.frmMain_BackupShowTime_After + TimeTypeUtil.getMinShowStr( MinsSplite );
end;
...
begin
  ...
  DesItemData.IpStr := Ip;
  DesItemData.DesPcName := DesPcName;
...
procedure TDesItemAddNetworkFace.SetIp(_Ip, _DesPcName: string);
begin
  Ip := _Ip;
...
begin
  Result := MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_AutoBackup, MyBoolean.getBooleanStr( NodeData.IsAutoSync ) );
  if NodeData.IsAutoSync then
  begin
    Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_AutoBackupInterval, TimeTypeUtil.getTimeShow( NodeData.SyncTimeType, NodeData.SyncTimeValue ) );
    Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_NextAutoBackupTime, DateTimeToStr( NodeData.NextSyncTime ) );
  end;
  Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_BackupAll, MyBoolean.getBooleanStr( NodeData.IsBackupNow ) );
  Result := Result + '<br />';
end;
...
begin
  ...
  Result := MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_BackupItem, NodeData.ShowName );
  Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_Destination, DestinationStr );
  Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_LastBackupTime, DateTimeToStr( NodeData.LastSyncTime ) );
  Result := Result + '<br />';
end;
...
begin
  Result := '';
  Result := MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_Encrypted, MyBoolean.getBooleanStr( NodeData.IsEncrypted ) );
  if NodeData.IsEncrypted then
    Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_PasswordHints, NodeData.PasswordHint );
end;
...
begin
  ...
  begin
    Result := Result + '<br />';
    Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_IncludeFilter, '' );
  ...
  begin
    if Result <> '' then
      Result := Result + '<br />';
    Result := Result + '<br />';
    Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_ExcludeFilter, '' );
...
begin
  Result := '';
  Result := MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_SaveDeletedFiles, MyBoolean.getBooleanStr( NodeData.IsSaveDeleted ) );
  if NodeData.IsSaveDeleted then
    Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_BackupHint_SaveDeletedEditions, IntToStr( NodeData.SaveDeletedCount ) );
  Result := Result + '<br />';
end;
...
begin
  frmBackupLog.lbStatus.Caption := LanguageInfo.frmMain_LogStatus_Busy;
...
begin
  frmBackupLog.lbStatus.Caption := LanguageInfo.frmMain_LogStatus_NotConn;
  frmBackupLog.plStatus.Visible := True;
end;
...
begin
  frmBackupLog.lbStatus.Caption := LanguageInfo.frmMain_LogStatus_NotExist;
  frmBackupLog.plStatus.Visible := True;
end;
...
{ TDesItemSetAccountFace }

procedure TDesItemSetAccountFace.AddPath(Path: string);
begin
  AccountPathList.Add( Path );
end;

constructor TDesItemSetAccountFace.Create;
begin
  inherited;
  AccountPathList := TStringList.Create;
end;

destructor TDesItemSetAccountFace.Destroy;
begin
  AccountPathList.Free;
  inherited;
end;

procedure TDesItemSetAccountFace.Update;
var
  DesNode, BackupNode : PVirtualNode;
  NodeData : PVstBackupData;
  HasBackupItem, IsItemVisible : Boolean;
begin
  inherited;
  ...
  while Assigned( DesNode ) do
  begin
    BackupNode := DesNode.FirstChild;
    while Assigned( BackupNode ) do
    begin
      NodeData := VstBackup.GetNodeData( BackupNode );
      IsItemVisible := AccountPathList.IndexOf( NodeData.ItemID ) >= 0;
      VstBackup.IsVisible[ BackupNode ] := IsItemVisible;
      HasBackupItem := HasBackupItem or IsItemVisible;
      BackupNode := BackupNode.NextSibling;
    end;
    DesNode := DesNode.NextSibling;
  end;

    // 是否存在 BackupItem
  frmMainForm.tbtnBackupAll.Visible := HasBackupItem;
end;

{ TAccountHideBackupBtnFace }

procedure TAccountHideBackupBtnFace.Update;
begin
  inherited;
  frmMainForm.tbtnBackupAll.Visible := False;
end;
