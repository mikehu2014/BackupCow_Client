...
uses UMainForm, UFOrmRestoreExplorer, UFormRestorePcFilter, UFormPreview, ULanguageConst;
...
begin
  ...
  if NodeData.NodeStatus = LanguageInfo.frmMain_RestoreNodeStatus_WaitingRestore then
    Result := MyShellTransActionIconUtil.getWaiting
  else
  if NodeData.NodeStatus = LanguageInfo.frmMain_RestoreNodeStatus_Analyizing then
    Result := MyShellTransActionIconUtil.getAnalyze
  else
  if NodeData.NodeStatus = LanguageInfo.frmMain_RestoreNodeStatus_Restoreing then
...
begin
  ...
  if NodeData.IsCompleted then
    Result := LanguageInfo.frmMain_RestoreStatusShow_Completed
  else
  if not NodeData.IsOnline then
    Result := LanguageInfo.frmMain_RestoreStatusShow_PcOffline
  else
  if not NodeData.IsRestoreExist then
    Result := LanguageInfo.frmMain_RestoreStatusShow_NotExist
  else
  if not NodeData.IsWrite then
    Result := LanguageInfo.frmMain_RestoreStatusShow_NotWrite
  else
  if NodeData.IsLackSpace then
    Result := LanguageInfo.frmMain_RestoreStatusShow_NotSpace
  else
  if NodeData.IsDesBusy then
    Result := LanguageInfo.frmMain_RestoreStatusShow_DesBusy
  else
  if not NodeData.IsConnected then
    Result := LanguageInfo.frmMain_RestoreStatusShow_NotConnect
  else
  if NodeData.NodeStatus = LanguageInfo.frmMain_RestoreNodeStatus_WaitingRestore then
    Result := LanguageInfo.frmMain_RestoreNodeStatus_WaitingRestore
  else
  if NodeData.NodeStatus = LanguageInfo.frmMain_RestoreNodeStatus_Analyizing then
  begin
    if NodeData.AnalyzeCount > 0 then
      Result := Format( LanguageInfo.frmMain_RestoreStatusShow_Analyizing, [ MyCount.getCountStr( NodeData.AnalyzeCount ) ] )
    else
  ...
  begin
    ...
    Result := LanguageInfo.frmMain_RestoreStatusShow_Incompleted;
end;
...
begin
  ...
  Result := MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_RestoreHint_ItemToRestore,  RestoreFrom );
  Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_RestoreHint_Owner,  NodeData.OwnerName );
  Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_RestoreHint_RestorePath,  NodeData.ShowName );
  Result := Result + MyHtmlHintShowStr.getHintRowNext( LanguageInfo.frmMain_RestoreHint_Encrypted, MyBoolean.getBooleanStr( NodeData.IsEncrypted ) );
  Result := Result + MyHtmlHintShowStr.getHintRow( LanguageInfo.frmMain_RestoreHint_SaveDeletedFiles, MyBoolean.getBooleanStr( NodeData.IsSaveDeleted ) );
end;
...
begin
  ...
  lbStatus.Caption := LanguageInfo.frmMain_ExplorerStatus_Busy;
...
begin
  ...
  lbStatus.Caption := LanguageInfo.frmMain_ExplorerStatus_NotConn;
  plStatus.Visible := True;
end;
...
begin
  inherited;
  frmPreView.lbStatus.Caption := LanguageInfo.frmMain_ExplorerStatus_Busy;
...
begin
  inherited;
  frmPreView.lbStatus.Caption := LanguageInfo.frmMain_ExplorerStatus_NotConn;
  frmPreView.plStatus.Visible := True;
end;
...
begin
  inherited;
  frmPreView.lbStatus.Caption := LanguageInfo.frmMain_ExplorerStatus_NotPreview;
  frmPreView.plStatus.Visible := True;
end;
...
begin
  inherited;
  frmPreView.lbStatus.Caption := LanguageInfo.frmMain_ExplorerStatus_Encrypted;
  frmPreView.plStatus.Visible := True;
end;
